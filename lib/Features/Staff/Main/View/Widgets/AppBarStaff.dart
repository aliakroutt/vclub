...
class MainAppBarStaff extends StatefulWidget implements PreferredSizeWidget {
  final ThemeService themeService;
  final VoidCallback onLogout;
  final VoidCallback onHomeTap;

  const MainAppBarStaff({
    super.key,
  ...
  @override
  State<MainAppBarStaff> createState() => _MainAppBarStaffState();

  @override
  Size get preferredSize => const Size.fromHeight(70);
}

class _MainAppBarStaffState extends State<MainAppBarStaff>
  ...
  void initState() {
    ...
    _themeRotation = Tween<double>(begin: 0, end: 0.5).animate(
      CurvedAnimation(parent: _themeAnimController, curve: Curves.easeInOutBack),
    );
  }
            ...
            child: BackdropFilter(
              ...
              child: AnimatedContainer(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      // ── Home button (replaces drawer/menu) ──
                      _HomeButton(isDark: isDark, onTap: widget.onHomeTap),
                      const Spacer(),
                      // ── Theme toggle ─────────────────────
...
// ─────────────────────────────────────────────────────────────
//  Home button (replaces the drawer menu button for staff)
// ─────────────────────────────────────────────────────────────
    ...
    return GestureDetector(
      ...
      child: ScaleTransition(
        ...
        child: Container(
          ...
          child: Icon(Iconsax.home_2, size: 19, color: AppColors.primary),
        ),
      ),
    );
  }
...
class _AppBarButtonState extends State<_AppBarButton>
  ...
  Color get _iconColor {
    ...
    return widget.isDark ? Colors.white.withOpacity(0.65) : Colors.black.withOpacity(0.5);
  }
  ...
  Color get _bgColor {
    ...
    return widget.isDark ? Colors.white.withOpacity(0.07) : Colors.black.withOpacity(0.04);
  }

  Color get _borderColor {
    if (widget.isDanger) return Colors.redAccent.withOpacity(0.25);
    return widget.isDark ? Colors.white.withOpacity(0.10) : Colors.black.withOpacity(0.07);
  }
