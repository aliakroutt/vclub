...
class _PermMeta {
  ...
  final String payloadKey; // key expected by the API
  final IconData icon;
...
class EmployeeFormBottomSheet extends StatefulWidget {
  ...
  @override
  State<EmployeeFormBottomSheet> createState() =>
      _EmployeeFormBottomSheetState();
}
...
class _EmployeeFormBottomSheetState extends State<EmployeeFormBottomSheet> {
  ...
  final Map<String, bool> _permissions = {
    for (final p in _kPerms) p.key: false,
  };
  ...
  bool _validate() {
    ...
    // Password is required on create, optional on edit (only if being changed)
  ...
  Future<void> _submit() async {
    ...
    if (success) {
      Get.back();
      AppSnackBar.success(
        widget.isEdit ? "employee_updated".tr : "employee_created".tr,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // ── Height cap so the sheet never behaves like a full-screen page ──
    return FractionallySizedBox(
      heightFactor: 0.85, // tweak this to taste (e.g. 0.75 for even smaller)
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: Icon(
                          widget.isEdit ? Iconsax.edit_2 : Iconsax.user_add,
                          color: primary,
                    ...
                    children: List.generate(_kPerms.length, (i) {
                      ...
                      return Padding(
                        padding: EdgeInsets.only(
                          bottom:
                              i < _kPerms.length - 1 ? size.height * 0.011 : 0,
                        ),
...
// ─────────────────────────────────────────────────────────────────────────────
// PERMISSION TILE (now stateless, controlled by parent)
// ─────────────────────────────────────────────────────────────────────────────
