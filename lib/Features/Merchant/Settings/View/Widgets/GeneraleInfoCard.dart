          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Iconsax.building_3, color: _accent, size: size.width * .052),
              ),
              SizedBox(width: size.width * .035),
              Expanded(child: AppText("general_information".tr, fontSize: size.width * .042, fontWeight: FontWeight.w700)),
            ],
          ),
          SizedBox(height: size.height * .025),

          _ModernField(controller: controller.companyController, label: "company_name".tr, icon: Iconsax.building),
          SizedBox(height: size.height * .018),
          _ModernField(controller: controller.tradeNameController, label: "trade_name".tr, icon: Iconsax.tag),
          SizedBox(height: size.height * .018),
          _ModernField(controller: controller.siretController, label: "siret".tr, icon: Iconsax.document_text),
          SizedBox(height: size.height * .018),
          _ModernField(controller: controller.industryController, label: "industry".tr, icon: Iconsax.category),
          SizedBox(height: size.height * .018),
          PhoneField(controller: controller.phoneController, label: "phone".tr),

          SizedBox(height: size.height * .022),
          Obx(() => SectionUpdateButton(loading: controller.savingGeneralInfo.value, onTap: controller.saveGeneralInfo)),
        ],
      ),
    );
  }
